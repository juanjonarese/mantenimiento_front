...
export default function EditarTrabajoModal({ trabajo, onClose, onGuardado }) {
  ...
  async function handleGuardar() {
    ...
    try {
      const items = [];
      [['SENDAS', form.sendas], ['RAMPAS', form.rampas], ['CORDONES', form.cordones]].forEach(([tipo, valor]) => {
        const superficie = numVal(valor);
        if (superficie <= 0) return;
        // Conservamos fotos/materiales/largo/ancho/cantidad del item original: solo se edita la superficie acá.
        const original = (trabajo.items || []).find((i) => i.tipoTrabajo === tipo);
        items.push({ ...(original || {}), tipoTrabajo: tipo, superficie });
      });
