...
export const obtenerAccesos = async (filtros = {}) => {
  const token = localStorage.getItem("token");
  const params = new URLSearchParams(filtros).toString();
  const response = await fetch(`${API_URL}/accesos${params ? `?${params}` : ""}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return handleResponse(response);
};
...
export const subirFoto = async (data, nombre, tipo, calle1 = '', calle2 = '') => {
  ...
  if (tipo?.startsWith('video/')) {
    ...
    formData.append('video', data, nombre);
    formData.append('nombre', nombre);
    if (calle1) formData.append('calle1', calle1);
    if (calle2) formData.append('calle2', calle2);
    const response = await fetch(`${API_URL}/fotos/upload-video`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: formData,
    });
...
export const eliminarFoto = async (url) => {
  const token = localStorage.getItem("token");
