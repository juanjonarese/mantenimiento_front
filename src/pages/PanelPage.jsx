...
export default function PanelPage() {
  ...
  // m² por familia de pintura, según los tipos de tarea asociados en el catálogo
  // a cada material "Pintura Termoplastica *" / "Pintura Acrilica *"
  const m2PorFamiliaPintura = (() => {
    const tiposDe = (match) => new Set(
      catalogo
        .filter((m) => normStrGlobal(m.nombre).includes(match))
        .flatMap((m) => (m.tiposTarea || []).map(normTipoGlobal))
    );
    const tiposTermo = tiposDe('termoplast');
    const tiposAcrilica = tiposDe('acrilic');
    let termoplastica = 0;
    let acrilica = 0;
    trabajos.forEach((t) => {
      const items = t.items?.length ? t.items : [{ tipoTrabajo: t.tipoTrabajo, superficie: t.superficie }];
      items.forEach((item) => {
        const tipo = normTipoGlobal(item.tipoTrabajo);
        const sup = item.superficie || 0;
        if (tiposTermo.has(tipo)) termoplastica += sup;
        if (tiposAcrilica.has(tipo)) acrilica += sup;
      });
    });
    return {
      termoplastica: parseFloat(termoplastica.toFixed(1)),
      acrilica: parseFloat(acrilica.toFixed(1)),
    };
  })();
    ...
    <div className="panel-page">
      ...
      <div className="container-fluid px-3 px-lg-4 py-3">
        ...
        <div className="row g-3 mb-4">
          <div className="col-6 col-md-4 col-lg-2">
            <div className="card border-0 h-100" style={{ borderLeft: '4px solid var(--bs-primary)' }}>
              <div className="card-body py-3 px-3">
                <div className="d-flex align-items-center gap-2 mb-1">
                  <i className="bi bi-collection text-primary"></i>
                  <span className="small text-muted">Total trabajos</span>
                </div>
                <div className="fw-bold fs-4 text-primary">{trabajos.length}</div>
                <div className="d-flex justify-content-between small text-muted mt-1">
                  <span>Termoplástica</span>
                  <span className="fw-semibold">{m2PorFamiliaPintura.termoplastica.toLocaleString('es-AR')} m²</span>
                </div>
                <div className="d-flex justify-content-between small text-muted mt-1">
                  <span>Acrílica</span>
                  <span className="fw-semibold">{m2PorFamiliaPintura.acrilica.toLocaleString('es-AR')} m²</span>
                </div>
              </div>
            </div>
          </div>
