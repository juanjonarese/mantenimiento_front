...
import { obtenerClientes, subirFoto, sincronizarTrabajos, obtenerTiposTarea, eliminarFoto } from '../services/api';
...
export default function NuevoTrabajoPage() {
  ...
  // URLs de fotos quitadas en el modal: se borran de R2 solo si se confirma con "Guardar"
  const [fotosABorrar, setFotosABorrar] = useState([]);
  ...
  // Borra una URL de R2 (best-effort): no frena la UI ni rompe si está offline
  // o si la URL no es de R2 (ej. una vieja de Cloudinary).
  async function borrarUrlR2(url) {
    if (!url || !navigator.onLine) return;
    try {
      await eliminarFoto(url);
    } catch (err) {
      console.warn('No se pudo borrar el archivo de R2:', err.message);
    }
  }

  function quitarFotoExistente(idx) {
    borrarUrlR2(fotosExistentes[idx]?.driveUrl);
    setFotosExistentes((prev) => prev.filter((_, j) => j !== idx));
  }

  function quitarFotoNueva(idx) {
    borrarUrlR2(fotos[idx]?.driveUrl);
    setFotos((prev) => prev.filter((_, j) => j !== idx));
  }

  // En el modal NO se borra al instante: se marca la URL como pendiente y se
  // borra de R2 recién al confirmar con "Guardar". Si se cancela, no se borra.
  function eliminarFotoModal(idx) {
    const foto = (modalForm.fotos || [])[idx];
    if (foto?.driveUrl) setFotosABorrar((prev) => [...prev, foto.driveUrl]);
    setModalForm((prev) => ({ ...prev, fotos: prev.fotos.filter((_, i) => i !== idx) }));
  }

  // Cancelar/cerrar el modal: descarta los cambios y NO borra nada de R2.
  ...
  function guardarModal() {
    ...
    // Confirmado: ahora sí borramos de R2 las fotos que se quitaron en el modal.
    fotosABorrar.forEach(borrarUrlR2);
    setFotosABorrar([]);
    setModalAbierto(false);
  }
  // ──────────────────────────────────────────────────────
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const trabajo = {
        ...
        usuario: localStorage.getItem('nombre') || localStorage.getItem('email') || '',
      ...
      if (rol === 'supervisor') {
        ...
        await Swal.fire({
          icon: 'success',
          title: esEdicion ? '¡Trabajo actualizado!' : '¡Trabajo guardado!',
          text: `${form.calle1} y ${form.calle2}`,
          timer: 1800,
          showConfirmButton: false,
        });
        navigate('/lista');
                  ...
                  {fotosExistentes.map((f, i) => (
                    <div key={i} className="position-relative">
                      ...
                      <button type="button" className="btn btn-danger btn-sm position-absolute top-0 end-0 p-0"
                        ...
                        onClick={() => quitarFotoExistente(i)}>×</button>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="d-flex flex-wrap gap-2">
                  {fotos.map((f, i) => (
                    <div key={i} className="position-relative">
                      ...
                      <button type="button" className="btn btn-danger btn-sm position-absolute top-0 end-0 p-0"
                        style={{ width: 18, height: 18, fontSize: 10, lineHeight: 1 }}
                        onClick={() => quitarFotoNueva(i)}>×</button>
                    </div>
                  ))}
                </div>
              </div>
