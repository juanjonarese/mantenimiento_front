...
export default function ListaPage() {
  ...
  const filtrados = trabajos.filter((t) => {
    ...
    const coincideUsuario = !filtroUsuario || t.usuario === filtroUsuario;
    const fecha = new Date(t.fechaCarga);
  ...
  // Orden fijo de columnas; tipos no listados aquí se agregan al final, alfabéticamente
  const ORDEN_TIPOS = ['SENDAS', 'LINEA DETENCION', 'CORDONES', 'CORDONES AMARILLO', 'CORDONES BLANCOS', 'RAMPAS'];

  // Tipos de tarea que realmente tienen datos entre los trabajos filtrados (sin movimiento no se muestran)
  const tiposConDatos = (() => {
    const set = new Set();
    filtrados.forEach((t) => {
      if (t.items?.length) {
        t.items.forEach((i) => { if (i.tipoTrabajo && (i.superficie || 0) > 0) set.add(i.tipoTrabajo); });
      } else if (t.tipoTrabajo && (t.superficie || 0) > 0) {
        set.add(t.tipoTrabajo);
      }
    });
    return [...set].sort((a, b) => {
  ...
  function exportarExcel() {
    const filas = filtrados.map((t) => ({
      ...
      'Calle 1':            t.calle1,
      'Calle 2':            t.calle2,
      ...Object.fromEntries(tiposConDatos.map((tipo) => [`${tipo} m²`, getSup(t, tipo) || ''])),
    ...
    <div className="lista-page">
      ...
      <div className="page-header bg-white border-bottom px-3 px-lg-4 py-3">
        ...
        <div className="row g-2">
          ...
          {!esCliente && (
            <div className="col-6 col-md-2">
              <select className="form-select form-select-sm" value={filtroUsuario}
                onChange={(e) => setFiltroUsuario(e.target.value)}>
                <option value="">Todos los usuarios</option>
                {[...new Set(trabajos.map((t) => t.usuario).filter(Boolean))]
                  .sort((a, b) => (resolve(a) || '').localeCompare(resolve(b) || ''))
                  .map((u) => (
                    <option key={u} value={u}>{resolve(u)}</option>
                  ))}
              </select>
            </div>
          )}
          ...
          <>
            ...
            <div className="d-flex gap-3 flex-wrap mb-3 px-1 small text-muted fw-semibold">
              {tiposConDatos.map((tipo) => (
                <span key={tipo}>{tipo}: <span className="text-primary">{filtrados.reduce((a, t) => a + getSup(t, tipo), 0).toFixed(1)} m²</span></span>
              ))}
            </div>
                  ...
                  <div className="card">
                    <div className="card-body py-2 px-3">
                      ...
                      <div className="d-flex gap-3 flex-wrap mb-2" style={{ fontSize: 12 }}>
                        {tiposConDatos.map((tipo) => {
                          const val = getSup(t, tipo);
                          return val > 0 && (
                            <span key={tipo} className="text-muted">{tipo} <strong>{val.toFixed(1)}</strong> m²</span>
                          );
                        })}
                      </div>
                ...
                <table className="table table-sm table-hover align-middle mb-0" style={{ fontSize: 13 }}>
                  ...
                  <tbody>
                    {paginados.map((t) => (
                      <tr key={t.id}>
                        ...
                        {tiposConDatos.map((tipo) => {
                          const val = getSup(t, tipo);
                          return <td key={tipo} className="text-end">{val > 0 ? val.toFixed(1) : <span className="text-muted">—</span>}</td>;
                        })}
                  ...
                  <tfoot className="table-light fw-semibold" style={{ fontSize: 12 }}>
                    <tr>
                      ...
                      {tiposConDatos.map((tipo) => (
                        <td key={tipo} className="text-end">{filtrados.reduce((a, t) => a + getSup(t, tipo), 0).toFixed(1)}</td>
                      ))}
