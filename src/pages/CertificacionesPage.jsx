...
import { obtenerTrabajos, importarDesdeBackend } from '../db/db';
import { obtenerTrabajosBackend, actualizarTrabajoBackend } from '../services/api';
...
// Mapeo de tipos de trabajo (catálogo TiposTarea) a columnas del certificado
const CERT_ITEMS = [
  { key: 'item1', lines: ['ÍTEM 1', 'Sendas Peat./', 'Lín. Detención', '(m²)'],  tipos: ['SENDAS', 'LINEA DETENCION', 'Senda peatonal', 'Línea divisoria', 'Flecha', 'Estacionamiento'] },
  { key: 'item2', lines: ['ÍTEM 2', 'Rampas para', 'Discapacitados', '(m²)'],    tipos: ['RAMPAS', 'Rampa', 'Ochava'] },
  { key: 'item3', lines: ['ÍTEM 3', 'Cordones', 'Cuneta', '(m²)'],               tipos: ['CORDONES', 'CORDONES BLANCOS', 'Cordón'] },
...
function agruparPorInterseccion(trabajos) {
  ...
  trabajos.forEach((t) => {
    ...
    (t.items || []).forEach((item) => {
      const tipo = normalizarTipo(item.tipoTrabajo);
      const ci = CERT_ITEMS.find((c) => c.tipos.some((tp) => normalizarTipo(tp) === tipo));
...
function descargarExcelCert(trabajos, certData = {}) {
  ...
  const aoa = [
    ...
    ...Array.from({ length: Math.max(20, inters.length) }, (_, i) => {
      const r = inters[i];
...
function imprimirPDF(trabajos, certData = {}) {
  ...
  const filasImprimir = Array.from({ length: Math.max(20, inters.length) }, (_, i) => inters[i] || null);
  const colHeaders = CERT_ITEMS.map((ci) => `<th class="num">${ci.lines.join('<br>')}</th>`).join('');
  const filasHTML = filasImprimir.map((r, i) => `
    <tr class="${r ? '' : 'vacia'}">
...
export default function CertificacionesPage() {
  ...
  const [filtroFechaCert, setFiltroFechaCert] = useState('');
  ...
  const filtrados = trabajos.filter((t) => {
    ...
    if (filtroFechaCert && fechaISOLocal(t.fechaCertificacion) !== filtroFechaCert) return false;
    return true;
  });

  const fechasCertificacion = [...new Set(trabajos.map((t) => fechaISOLocal(t.fechaCertificacion)).filter(Boolean))].sort().reverse();
  const trabajosDeLaFechaSeleccionada = filtroFechaCert ? trabajos.filter((t) => fechaISOLocal(t.fechaCertificacion) === filtroFechaCert) : [];
  ...
  async function actualizarEstado(trabajo, nuevoEstado, extras = {}) {
    const id = trabajo._id || trabajo.id;
    const cambios = {
      estadoAdmin: nuevoEstado,
      // Solo se fija/actualiza la fecha de certificación al certificar. Otras
      // transiciones (facturar, rechazar, enviar a revisión) no deben pisarla:
      // el historial de cuándo se certificó tiene que quedar intacto siempre.
      ...(nuevoEstado === 'Certificado' ? { fechaCertificacion: new Date().toISOString() } : {}),
      ...extras,
    };
    try {
      await actualizarTrabajoBackend(id, cambios);
    } catch (err) {
      setErrorModal(err.message || 'No se pudo guardar el cambio. Verificá tu conexión.');
      throw err;
    }
    await cargar();
  }
    ...
    <div className="certificaciones-page">
      ...
      <div className="px-3 px-lg-4 py-2 bg-white border-bottom d-flex align-items-center gap-2 flex-wrap">
        ...
        <select
          ...
          value={filtroFechaCert}
          onChange={(e) => { setFiltroFechaCert(e.target.value); setExpandido(null); }}
        >
          <option value="">Todas las fechas de certificación</option>
          {fechasCertificacion.map((fecha) => (
            <option key={fecha} value={fecha}>{isoToDisplay(fecha)}</option>
          ))}
        </select>
        {filtroFechaCert && (
          <button
            className="btn btn-sm btn-outline-primary"
            onClick={() => setModal({ tipo: 'detalleCertificado' })}
          >
            <i className="bi bi-file-earmark-text me-1"></i>Ver detalle del {isoToDisplay(filtroFechaCert)}
          </button>
        )}
      </div>
        ...
        >
          ...
          >
            <div className="modal-content">
              ...
              {/* ── MODAL DETALLE DE CERTIFICACIÓN POR FECHA ── */}
              {modal.tipo === 'detalleCertificado' && (() => {
                const grupo = trabajosDeLaFechaSeleccionada;
                const inters = agruparPorInterseccion(grupo);
                const totales = Object.fromEntries(CERT_ITEMS.map((ci) => [ci.key, 0]));
                inters.forEach((r) => CERT_ITEMS.forEach((ci) => { totales[ci.key] += r.totales[ci.key]; }));
                const supTotal = grupo.reduce((s, t) => s + parseFloat(superficieDe(t)), 0).toFixed(2);
                const nros = [...new Set(grupo.map((t) => t.nroCertificado).filter(Boolean))];
                const estadosPresentes = [...new Set(grupo.map((t) => t.estadoAdmin || 'Sin certificar'))];
                const certDataGrupo = { nro: nros.join(', '), expediente: '', contrato: '', periodo: '', inspector: '' };
                  ...
                  <>
                    <div className="modal-header bg-primary text-white">
                      <h6 className="modal-title">
                        ...
                        Certificaciones del {isoToDisplay(filtroFechaCert)}
                      </h6>
                      <button className="btn-close btn-close-white" onClick={() => setModal(null)} />
                    </div>
                    ...
                    <div className="modal-body p-0" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                      ...
                      <div className="px-3 pt-3 pb-2 border-bottom">
                        <div className="fw-semibold small mb-2 text-muted text-uppercase" style={{ letterSpacing: '.5px' }}>
                          <i className="bi bi-card-text me-1"></i>Datos generales
                        </div>
                        <div className="d-flex align-items-center gap-2 flex-wrap mb-2">
                          {estadosPresentes.map((e) => {
                            const c = ESTADO_CONFIG[e] || ESTADO_CONFIG['Sin certificar'];
                            return (
                              <span key={e} className={`badge bg-${c.color} text-${c.color === 'warning' ? 'dark' : 'white'}`}>
                                <i className={`bi bi-${c.icon} me-1`}></i>{c.label}
                              </span>
                            );
                          })}
                          <span className="badge bg-primary bg-opacity-75">{supTotal} m²</span>
                          <span className="badge bg-secondary bg-opacity-75">{grupo.length} trabajo{grupo.length !== 1 ? 's' : ''}</span>
                        </div>
                        {nros.length > 0 && (
                          <div className="small"><span className="text-muted">N° Certificado:</span> <strong>{nros.join(', ')}</strong></div>
                        )}
                      </div>
                      ...
                      <div className="px-3 pt-3 pb-2 border-bottom">
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          ...
                          <div className="d-flex gap-2">
                            <button className="btn btn-sm btn-outline-success"
                              onClick={() => descargarExcelCert(grupo, certDataGrupo)}>
                              <i className="bi bi-file-earmark-excel me-1"></i>Excel
                            </button>
                            <button className="btn btn-sm btn-outline-dark"
                              onClick={() => imprimirPDF(grupo, certDataGrupo)}>
                              <i className="bi bi-printer me-1"></i>PDF
                            </button>
                          </div>
                        </div>
                          ...
                          <table className="table table-sm table-bordered mb-0" style={{ fontSize: 11, minWidth: 500 }}>
                            ...
                            <tfoot className="fw-bold" style={{ background: '#dce6f1' }}>
                              <tr>
                                <td colSpan={2} className="text-center">TOTALES</td>
                                {CERT_ITEMS.map((ci) => (
                                  <td key={ci.key} className="text-end text-primary">{totales[ci.key].toFixed(2)}</td>
                                ))}
                              </tr>
                            </tfoot>
                          </table>
                      ...
                      <div className="px-3 py-3">
                        <div className="fw-semibold small mb-2 text-muted text-uppercase" style={{ letterSpacing: '.5px' }}>
                          <i className="bi bi-list-check me-1"></i>Trabajos incluidos
                        </div>
                        <ul className="list-unstyled small mb-0">
                          {grupo.map((t) => {
                            const e = t.estadoAdmin || 'Sin certificar';
                            const c = ESTADO_CONFIG[e] || ESTADO_CONFIG['Sin certificar'];
                            return (
                              <li key={t.id} className="d-flex align-items-center gap-2 py-2 border-bottom flex-wrap">
                                <span className={`badge bg-${c.color} text-${c.color === 'warning' ? 'dark' : 'white'}`} style={{ fontSize: 10 }}>
                                  {c.label}
                                </span>
                                <span className="fw-semibold">{t.calle1} y {t.calle2}</span>
                                {t.nroCertificado && <span className="text-muted">N° {t.nroCertificado}</span>}
                                {t.expedienteMunicipal && <span className="text-muted">Exp. {t.expedienteMunicipal}</span>}
                                <span className="text-muted">{superficieDe(t)} m²</span>
                                <span className="text-muted ms-auto text-nowrap">{formatFecha(t.fechaCarga)}</span>
                                <Link to={`/detalle/${t.id}`} className="btn btn-sm btn-outline-secondary py-0 px-2" onClick={(e2) => e2.stopPropagation()}>
                    ...
                    <div className="modal-footer">
                      <button className="btn btn-outline-secondary" onClick={() => setModal(null)}>Cerrar</button>
                    </div>
                  </>
                );
              })()}
